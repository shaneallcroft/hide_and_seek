...
public:
  PeopleFinder() { //maybe intake an integer number of players
    ...
    found_count = 0;
    legs_found = false;
    current_location_tracker = n.subscribe("amcl_pose", 10, &PeopleFinder::current_pos_retriever,
        this);
    ...
    person_leg_detector = n.subscribe("leg_detector/leg_tracker_measurements", 10,
        &legs_detect_callback, this);
    person_face_detector = n.subscribe("face_detector/people_tracker_measurements", 10,
        &face_detect_callback, this);
  }

  void current_pos_retriever(
    const geometry_msgs::PoseWithCovarianceStamped::ConstPtr & current_pos);
  void PeopleFinder::legs_detect_callback(
    const people_msgs::PositionMeasurementArray::ConstPtr & leg_place);
  void PeopleFinder::face_detect_callback(
    const people_msgs::PositionMeasurementConstPtr & face_place);
...
void PeopleFinder::current_pos_retriever(
  const geometry_msgs::PoseWithCovarianceStamped::ConstPtr & current_pos)
{

...
public:
  ...
  void current_pos_retriever(
    const geometry_msgs::PoseWithCovarianceStamped::ConstPtr & current_pos);
...
void GoalSetter::current_pos_retriever(
  const geometry_msgs::PoseWithCovarianceStamped::ConstPtr & current_pos)                                       //I think the ConstPtr thing there is extraneous   LOL JK
{
  ...
  if ( (int)((current_position.pose.position.x - current_goal.pose.position.x) +
    (current_position.pose.position.y - current_goal.pose.position.y)) / PROXIMITY_THRESHOLD == 0)                                                                            // i.e. if(close enough) Since they're(x and y) both held to the same standard of closeness, we can add them together and divide by that standard (technically this should be divided by 2 again but threshold was pretty large before
  {
    this.set_goal();
  }
...
//find closest point and make that the new goal.
    ...
    float closest_distance =
      pow(pow((tp_at_current_location.tp_pos.x - ottpm.miniverse[i].tp_pos.x),
        2) + pow((tp_at_current_location.tp_pos.y - ottpm.miniverse[index_of_closest_tp].y), 2),
        .5);
    ...
    for (int i = 0; i < ottpm.response.miniverse_size; ++i) {
      ...
      float distance = pow(pow((tp_at_current_location.tp_pos.x - ottpm.miniverse[i].tp_pos.x),
          2) +
          pow((tp_at_current_location.tp_pos.y - ottpm.miniverse[index_of_closest_tp].y), 2), .5);
    ...
    ROS_INFO("New goal chosen at x=%ld, y=%ld" (long int)current_goal.target_pose.pose.position.x,
